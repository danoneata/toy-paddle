...
class KM(object):
    ...
    def run_task(self, task_dic):
        ...
        y_s = task_dic["y_s"]  # [n_task, shot, 1]
        y_q = task_dic["y_q"]  # [n_task, n_query, 1]
        ...
        query, support = self.scaler(query, support)

        # Run adaptation
        return self.run_method(support=support, query=query, y_s=y_s, y_q=y_q)
...
class Paddle(KM):
    ...
    def w_update(self, support, query, y_s_one_hot):
        ...
        num = (
            torch.einsum("bkq,bqd->bkd", torch.transpose(self.u, 1, 2), query) +
    ...
    def run_method(self, support, query, y_s, y_q):
        ...
        for i in tqdm(range(self.iter)):
            ...
            yield self.u, self.scaler.undo(self.w)
...
class MinMaxScaler(object):
    ...
    def __call__(self, query, support):
        ...
        dist[dist == 0.0] = 1.0
        self.scale = 1.0 / dist
        self.ratio = query.min(dim=1, keepdim=True)[0]
        query.mul_(self.scale).sub_(self.ratio)
        support.mul_(self.scale).sub_(self.ratio)
        return query, support

    def undo(self, X):
        return X.add(self.ratio).div(self.scale)